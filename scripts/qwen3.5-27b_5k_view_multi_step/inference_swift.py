...
6. 倒角与圆角的区分（重要）：
   - 倒角 (Chamfer)：斜面切削，边缘为直线段
     标注特征：含 C、D、A、° 字样（如 C1、D5xA45、1x45°）
   - 圆角 (Fillet)：圆弧过渡，边缘为弧线
     标注特征：含 R 字样（如 R3、R5）
   - 判断依据：优先看标注文字，R 开头 → 圆角，C/D 开头或含角度 → 倒角
...
def postprocess_features(detections: list) -> list:
    """Post-processing rules for structural features."""
    import re
    ...
    for det in detections:
        ...
        # Skip detections with invalid bbox
        if not bbox or len(bbox) != 4:
            continue

        # --- Size-based category correction ---
        # Chamfer↔Fillet disambiguation based on size string
        if cat in ("Chamfer", "Chamfer Group") and size:
            # If size starts with R (e.g., R3, R5), it's a fillet
            if re.match(r'^R\d', size):
                new_cat = "Fillet Group" if "Group" in cat else "Fillet"
                logger.debug(f"Reclassify {cat} -> {new_cat} (size={size})")
                det["category"] = new_cat
        elif cat in ("Fillet", "Fillet Group") and size:
            # If size contains C, D, A, or 45°, it's a chamfer
            if re.match(r'^[CD]\d', size) or 'x45' in size or '45°' in size:
                new_cat = "Chamfer Group" if "Group" in cat else "Chamfer"
                logger.debug(f"Reclassify {cat} -> {new_cat} (size={size})")
                det["category"] = new_cat

        # --- Hole type correction based on size ---
        if cat in ("Round Hole", "Round Hole Group") and size:
            if re.match(r'^M\d', size) or re.search(r'\d+xM\d', size):
                new_cat = "Threaded Hole Group" if "Group" in cat else "Threaded Hole"
                det["category"] = new_cat
            elif re.search(r'H[67]', size):
                new_cat = "Pin Hole Group" if "Group" in cat else "Pin Hole"
                det["category"] = new_cat

        corrected.append(det)

    # --- Soft NMS for chamfer/fillet overlap ---
    # If chamfer and fillet overlap with IoU > 0.7, keep only one
    final = []
    removed = set()
    for i, d1 in enumerate(corrected):
        if i in removed:
            continue
        for j, d2 in enumerate(corrected):
            if j <= i or j in removed:
                continue
            if not d1.get("bbox") or not d2.get("bbox"):
                continue
            c1, c2 = d1["category"], d2["category"]
            # Only apply between chamfer↔fillet pairs
            cf_set = {"Chamfer", "Fillet", "Chamfer Group", "Fillet Group"}
            if c1 in cf_set and c2 in cf_set and c1 != c2:
                iou = calculate_iou(d1["bbox"], d2["bbox"])
                if iou > 0.7:
                    # Keep the one whose size matches its category better
                    s1, s2 = d1.get("size", ""), d2.get("size", "")
                    # Prefer the one with a size string
                    if s1 and not s2:
                        removed.add(j)
                    elif s2 and not s1:
                        removed.add(i)
                    else:
                        removed.add(j)  # keep first by default
        if i not in removed:
            final.append(d1)
...
def main():
    ...
    parser.add_argument("--image_max_tokens", type=int, default=2560,
                        help="IMAGE_MAX_TOKEN_NUM for Qwen VL (must match training)")
    ...
    # Set IMAGE_MAX_TOKEN_NUM to match training resolution
    os.environ["IMAGE_MAX_TOKEN_NUM"] = str(args.image_max_tokens)
    logger.info(f"IMAGE_MAX_TOKEN_NUM={args.image_max_tokens}")
