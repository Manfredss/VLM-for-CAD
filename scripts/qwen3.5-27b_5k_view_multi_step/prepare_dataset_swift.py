...
# Oversampling multipliers for weak categories (based on platform eval)
# view_detection=55.2%, chamfer=58.7% are the weakest
OVERSAMPLE_MAP = {
    "Section View": 5,
    "Bill of Materials": 5,
    "Auxiliary View": 4,
...
def main():
    ...
    with open(output_dir / train_name, "w", encoding="utf-8") as f:
        for i in train_indices:
            ...
            # Oversample: repeat images containing weak categories
            labels = get_labels_in_item(valid_data[i])
            max_repeat = max(
                (OVERSAMPLE_MAP.get(lab, 1) for lab in labels), default=1
            )
    ...
    print(f"  Oversampled copies: {oversample_count}")
