...
Adapted for Qwen3.5-27B — 15 features + view detection (25 categories)

Metrics reported:
  - detection_f1:        overall F1 (IoU >= threshold + category match)
  - detection_f1_macro:  macro-averaged F1 across categories
...
Compatible with ms-swift 4.0.0.dev0, Qwen3.5-27B
"""
...
def compute_task_acc(preds, labels, *, acc_strategy='token',
    ...
    # Compute per-category F1 and macro-average
    cat_f1_values = []
    cat_metrics = {}
    for cat in ALL_CATEGORIES:
        ...
        # Only include categories that appear in GT for macro-average
        if (cat_tp + cat_fn) > 0:
