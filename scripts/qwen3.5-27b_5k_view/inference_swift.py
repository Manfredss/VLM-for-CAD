"""
inference_swift.py — Qwen3.5-27B inference for 15-feature + view detection

25 categories: 14 structural features + 11 view/layout elements.
Supports both standalone batch inference and checkpoint-based eval.
"""
...
# =====================================================================
# Prompts — 25 categories (15 features + views)
# =====================================================================
DEFAULT_SYSTEM_PROMPT = """任务：
在输入的工程图纸中定位、分类并提取以下结构特征及视图布局元素，并提取对应尺寸参数，输出 JSON 列表。
...
1.4 视图与布局元素（11 类）
  ...
  - 正交投影 - 主视图 (Orthographic Projection - Front View)
  - 正交投影 - 俯视图 (Orthographic Projection - Top View)
  - 正交投影 - 左视图 (Orthographic Projection - Left View)
  - 正交投影 - 右视图 (Orthographic Projection - Right View)
...
DETECTION_PROMPT = "请分析这张工程图纸，识别并提取所有结构特征和视图布局元素。"
...
def load_dataset_image_names(*dataset_paths: str) -> set:
    ...
    for path_str in dataset_paths:
        ...
        logger.info(f"Loaded {len(names) - before} image names from {dataset_path} (total unique: {len(names)})")

    return names


def main():
    parser = argparse.ArgumentParser(description="Qwen3.5-27B 15-feature + view batch inference")
    parser.add_argument("--model_path",    type=str, default="Qwen/Qwen3.5-27B")
    parser.add_argument("--adapter_path",  type=str, default="/workspace/output/swift_27b_15feats_view/final_adapter")
    parser.add_argument("--image_dir",     type=str, default="/workspace/data/IM_D03_PT_5K")
    parser.add_argument("--output_file",   type=str, default="/workspace/output/swift_27b_15feats_view_results.json")
    ...
    parser.add_argument("--train_dataset", type=str, default="/workspace/data/train_15feats_view.jsonl")
    parser.add_argument("--val_dataset",   type=str, default="/workspace/data/val_15feats_view.jsonl")
    ...
    if not _run_tag or _run_tag in ("output", "workspace"):
        _run_tag = "swift_27b_15feats_" + time.strftime("%Y%m%d_%H%M%S")
    ...
    if not whitelist:
        logger.error("Dataset whitelist is empty. Check --train_dataset/--val_dataset paths.")
        return

    before_filter = len(all_images)
    all_images = [p for p in all_images if p.name in whitelist]
    logger.info(f"After filtering: {len(all_images)} images (skipped {before_filter - len(all_images)})")
    ...
    if not remaining:
        ...
        for i, img_path in enumerate(remaining):
            ...
            if (global_idx + 1) % args.batch_size == 0:
                ...
                _save_batch(done_dict, all_images, batch_start, batch_end, output_file, batch_num)
    ...
    if tail_start < len(all_images) and len(done_dict) >= len(all_images):
        _save_batch(done_dict, all_images, tail_start, len(all_images), output_file, n_full_batches + 1)
...
def _save_batch(done_dict, all_images, start_idx, end_idx, output_file, batch_num):
    ...
    logger.info(f"  >>> Batch {batch_num:03d} saved (#{start_idx+1}~#{end_idx}, {len(batch_records)} images)")
