#!/bin/bash
# ================================================================
# Qwen3.5-27B ms-swift LoRA training — 7 features + 13 views (multi-turn)
#
# Two-turn conversation:
#   Turn 1: 13 view/document categories (no size)
#   Turn 2: 7 structural features (Round/Rectangular/Threaded/Slotted Hole,
#           Fillet, Bending, Silver Plating) — with size, same as silver_plate_bend
#
# Key settings (3× A100-80GB, scaled up from 2-GPU baseline F1=0.93):
#   - max_length=10240 (image=2560 + multi-turn outputs)
#   - image_max_token_num=2560 (1.6× resolution; helps small-text Notes + small features)
#   - LoRA rank=64 all-linear (extra capacity for 20 categories + 2-turn)
#   - DeepSpeed Zero-3
#   - Hierarchical LR: ViT 3e-5, Aligner 2e-5, LLM 2e-5
#   - Unlocked ViT + Aligner
#   - 6 epochs, eval/save every 50 steps
#   - per_device_batch=1, grad_acc=2 → effective batch 4 on 2 GPUs (small dataset, more updates)
#
# Usage (on training pod):
#   TRAIN_CUDA_VISIBLE_DEVICES=0,1 bash /workspace/scripts_27b_788_with_view/train_swift.sh
# ================================================================
...
TRAIN_DATASET="${TRAIN_DATASET:-/workspace/data/train_view_7feats.jsonl}"
VAL_DATASET="${VAL_DATASET:-/workspace/data/val_view_7feats.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/workspace/output/swift_27b_788_view_7feats}"
...
JANITOR_LOG="${JANITOR_LOG:-/workspace/logs/keep_best_n_ckpts_view_7feats.log}"
JANITOR_PID=""
_stop_janitor() {
