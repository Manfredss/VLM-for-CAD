"""
inference_swift.py — Multi-turn inference for views + 7 features.

Two generations per image:
  Turn 1: views/document layout (13 categories, no size)
  Turn 2: structural features (7 categories, with size; same as silver_plate_bend)

Loads base Qwen3.5-27B + LoRA adapter from training, runs inference on the
images listed in a JSONL split file. Per record, the saved JSON contains the
view list, the feature list, and the combined detection list.
"""
...
def _normalize_items(items, default_size=""):
    out = []
    for f in items:
        out.append({
...
# =====================================================================
def main():
    parser = argparse.ArgumentParser(description="Qwen3.5-27B views + 7-feature multi-turn inference")
    parser.add_argument("--model_path",    type=str, default="Qwen/Qwen3.5-27B")
    parser.add_argument("--adapter_path",  type=str, default="/workspace/output/swift_27b_788_view_7feats/final_adapter")
    parser.add_argument("--test_jsonl",    type=str, default="/workspace/data/test_view_7feats.jsonl")
    parser.add_argument("--image_dir",     type=str, default="/workspace/data/simens_7feats")
    parser.add_argument("--output_file",   type=str, default="/workspace/output/swift_27b_788_view_7feats_results.json")
    ...
    if remaining:
        ...
        for i, img_path in enumerate(remaining):
            ...
            try:
                ...
                view_items = _normalize_items(_deduplicate(parse_output(raw_views)), default_size="")
                feat_items = _normalize_items(_deduplicate(parse_output(raw_features)))
                error = None
            except Exception as e:
