#!/bin/bash
# ================================================================
# Qwen3.5-27B ms-swift LoRA SFT — Siemens 788 pipeline
# 15 layout/view labels + 10 features (incl. hole groups), multi-turn
#
# Merged best practices from:
#   - 788_with_view (baseline F1 ~0.93)
#   - 788_with_view_improve (oversampling, sharpened prompts, IMMetric)
#   - silver_plate_bend (per_device_batch=2 baseline)
#
# Key settings:
#   - max_length=12288, image_max_token_num=4096
#   - LoRA rank=64, alpha=128, all-linear
#   - DeepSpeed Zero-3
#   - Hierarchical LR: ViT 3e-5, Aligner 2e-5, LLM 2e-5
#   - Unlocked ViT + Aligner
#   - IMMetric for detection-aware eval, --use_logits_to_keep
...
VAL_DATASET="${VAL_DATASET:-/workspace/data/val_view_7feats_pipeline.jsonl}"
...
# --- Top-N checkpoint janitor (background) ---
JANITOR_LOG="${JANITOR_LOG:-/workspace/logs/keep_best_n_ckpts_pipeline.log}"
JANITOR_PID=""
_stop_janitor() {
