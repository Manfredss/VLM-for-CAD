...
# For Qwen3.5-4B with NEW prompt
#
# Usage:
#   nohup bash watch_eval_then_infer.sh > /workspace/finetune/logs/watch_4b_newprompt.log 2>&1 &
# ================================================================
set -euo pipefail
...
# Config
OUTPUT_DIR="${OUTPUT_DIR:-/workspace/finetune/output/swift_4b_newprompt}"
BENCHMARK_DATASET="${BENCHMARK_DATASET:-/workspace/finetune/data/data/benchmark_10feats_newprompt_4b.jsonl}"
...
DONE_FILE="${OUTPUT_DIR}/.done_eval_infer_4b_newprompt"

BENCHMARK_RESULTS="${OUTPUT_DIR}/benchmark_results_4b_newprompt.jsonl"
SAMPLED100_DIR="${OUTPUT_DIR}/sampled100_results_4b_newprompt"

mkdir -p "$SAMPLED100_DIR"
touch "$DONE_FILE"

echo "============================================="
echo " Checkpoint Watcher — 4B New Prompt"
...
while true; do
    ...
    for ckpt_dir in "$OUTPUT_DIR"/*/checkpoint-*; do
        ...
        echo ""
        echo ">>> Found new checkpoint: $full_name"
        echo ">>> $(date)"

        # Step 1: Benchmark eval
        echo ">>> [1/2] Running benchmark eval..."
        BENCH_OUT="${OUTPUT_DIR}/benchmark_${ckpt_name}_4b_newprompt.json"
...
")
            ...
            echo ">>> Benchmark done: $METRICS"
        else
            echo ">>> Benchmark eval FAILED for $ckpt_name"
        fi
        ...
        if python3 -u "$SCRIPT_DIR/infer_sampled100.py" \
            ...
            echo ">>> Inference FAILED for $ckpt_name"
        fi
