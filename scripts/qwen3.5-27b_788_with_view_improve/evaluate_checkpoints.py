...
What it does:
  ...
  - Scores predictions with the same IoU+category logic as metric.py
  - Reports per-checkpoint F1 (views, features, overall) and ranks them
...
def main():
    ...
    for ci, ckpt in enumerate(ckpts):
        ...
        view_total = {"tp": 0, "fp": 0, "fn": 0, "iou_sum": 0.0, "match": 0,
                      "size_correct": 0, "size_total": 0,
                      "per_cat": defaultdict(lambda: {"tp": 0, "fp": 0, "fn": 0})}
        feat_total = {"tp": 0, "fp": 0, "fn": 0, "iou_sum": 0.0, "match": 0,
                      "size_correct": 0, "size_total": 0,
                      "per_cat": defaultdict(lambda: {"tp": 0, "fp": 0, "fn": 0})}
        ...
        for i, rec in enumerate(val_items):
            ...
            pred_v = extract(parse_json_arrays(raw_v))
            pred_f = extract(parse_json_arrays(raw_f))
            merge(view_total, score(pred_v, extract(gt_v), score_size=False))
            merge(feat_total, score(pred_f, extract(gt_f), score_size=True))
        ...
        size_acc = (feat_total["size_correct"] / feat_total["size_total"]
                    if feat_total["size_total"] > 0 else 0.0)
        iou_v = view_total["iou_sum"] / view_total["match"] if view_total["match"] > 0 else 0.0
        iou_f = feat_total["iou_sum"] / feat_total["match"] if feat_total["match"] > 0 else 0.0
        ...
        result = {
            ...
            "views":    {"P": Pv, "R": Rv, "F1": Fv, "mean_iou": iou_v},
            "features": {"P": Pf, "R": Rf, "F1": Ff, "mean_iou": iou_f, "size_acc": size_acc},
            "overall":  {"P": Pc, "R": Rc, "F1": Fc},
            "per_cat_views": per_cat_views,
            "per_cat_features": per_cat_feats,
        }
        all_results[ckpt.name] = result
        logger.info(f"  → views F1={Fv:.4f}, features F1={Ff:.4f}, overall F1={Fc:.4f}")
    ...
    # Final summary
    print("\n=== SUMMARY (sorted by overall F1, descending) ===")
    print(f"{'ckpt':<20s} {'views F1':>10s} {'features F1':>13s} {'overall F1':>12s}")
    ranked = sorted(all_results.items(),
                    key=lambda kv: -kv[1]["overall"]["F1"])
    for name, r in ranked:
        print(f"{name:<20s} {r['views']['F1']:>10.4f} {r['features']['F1']:>13.4f} {r['overall']['F1']:>12.4f}")
    winner = ranked[0][0]
    print(f"\nWinner by overall F1: {winner}")
