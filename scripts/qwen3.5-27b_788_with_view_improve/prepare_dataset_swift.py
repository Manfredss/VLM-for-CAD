...
Two-step conversation:
  ...
  Turn 2 (Structural Features): 7 categories, with size
"""
...
DEFAULT_JSON_PATH = SCRIPT_DIR.parent / "qwen3.5-27b_788_with_view" / "788_11Feats_View.json"
...
4. **本数据集图纸全部为 ISO 第一角投影法**；不得按第三角投影解释视图位置。
5. 罕见类别（剖视图、详图、辅助视图、后视图）出现时必须独立检测；不得遗漏。

【一致性约束 — 必须满足】
6. 每张图纸至多 1 个 Front View。若有多个候选，仅最大且尺寸标注最丰富者为 Front View。
7. 投影法一致性（ISO 第一角）：
...
8. 同一组正交视图中，俯视图与仰视图不应同时出现（仅出现表达底面独特特征的一个）。
9. 等轴测图（如有）中可见的"正面"应与正视图轮廓一致；"顶面"应与俯视图轮廓一致。若不一致，请重新核对正视图分类。
10. 仅出现一个正交视图时：若展示折弯线和折弯标注 → Flat Pattern View；其他情况默认为 Front View。
