"""
inference_4b.py — Qwen3-VL-4B / Qwen3.5-4B 批量推理脚本 (Nautilus)

支持两种模式：
  1. 合并模型（--model_path 指向合并后目录，不传 --adapter_path）
  2. 基础模型 + LoRA adapter（同时传 --model_path 和 --adapter_path）

特性：
  - 断点续传（自动跳过已处理的图片）
  - 数据集白名单过滤（--train_dataset / --val_dataset / --test_dataset）
  - 传 --no_whitelist 跳过白名单，对全部图片推理
  - 每 N 张自动保存进度
  - 解析失败时保留原始文本，不中断流程
  - 进度条 + 速度估算

用法（合并模型）：
  python3 inference_4b.py \
      --model_path /workspace/finetune/output/qwen3-vl-4b-merged \
      --image_dir  /workspace/finetune/data/data/IM_D03_PT_5k_Augmented \
      --output_file /workspace/finetune/output/infer_results_4b.json \
      --train_dataset /workspace/finetune/data/data/train_v2.jsonl \
      --val_dataset   /workspace/finetune/data/data/val_v2.jsonl \
      --test_dataset  /workspace/finetune/data/data/test_v2.jsonl

用法（基础模型 + adapter）：
  python3 inference_4b.py \
      --model_path  Qwen/Qwen3-VL-4B-Instruct \
      --adapter_path /workspace/finetune/output/swift_4b/checkpoint-XXXX \
      --image_dir    /workspace/finetune/data/data/IM_D03_PT_5k_Augmented \
      --output_file  /workspace/finetune/output/infer_results_4b.json \
      --train_dataset /workspace/finetune/data/data/train_v2.jsonl \
      --val_dataset   /workspace/finetune/data/data/val_v2.jsonl
...
# =====================================================================
# Prompts — 与训练时一致（bbox_2d 归一化 0-1000 版本）
# =====================================================================
...
def load_model(model_path: str, adapter_path: str | None, load_in_4bit: bool):
    ...
    if adapter_path:
        ...
        logger.info("No adapter — using merged model directly")
...
def run_inference_single(model, processor, image_path: Path, max_new_tokens: int) -> str:
    ...
    # apply_chat_template — 对 Qwen3.5 关闭 thinking，对 Qwen3-VL 无影响
    try:
...
def _deduplicate(detections: list) -> list:
    ...
    for item in detections:
        key = (
            item.get("category"),
            item.get("size", ""),
            tuple(item.get("bbox_2d", item.get("bbox", []))),
        )
        if key not in seen:
            seen.add(key)
            result.append(item)
...
# 保存
# =====================================================================
def _save_batch(done_dict, all_images, start_idx, end_idx, output_file, batch_num):
...
# =====================================================================
def main():
    parser = argparse.ArgumentParser(description="Qwen3-VL-4B / Qwen3.5-4B batch inference")
    parser.add_argument("--model_path",     type=str, required=True,
                        help="合并模型路径 或 HuggingFace 模型 ID")
    parser.add_argument("--adapter_path",   type=str, default=None,
                        help="LoRA adapter 路径（不传则使用合并模型）")
    ...
    # 日志文件
    _tag = Path(args.model_path).name or "4b_inference"
    _log_dir = Path(f"/workspace/finetune/logs/{_tag}_{time.strftime('%Y%m%d_%H%M%S')}")
    ...
    if not args.no_whitelist:
        whitelist = load_dataset_image_names(
            args.train_dataset, args.val_dataset, args.test_dataset
        )
        if not whitelist:
            logger.error(
                "Dataset whitelist is empty. "
                "Check --train_dataset/--val_dataset/--test_dataset paths, "
                "or use --no_whitelist to skip filtering."
            )
            return
        before_filter = len(all_images)
        all_images = [p for p in all_images if p.name in whitelist]
        logger.info(
            f"Whitelist: {len(whitelist)} images | "
            f"After filter: {len(all_images)} (skipped {before_filter - len(all_images)})"
        )
    else:
        logger.info("Whitelist disabled — processing ALL images")
    ...
    if not remaining:
        ...
        for i, img_path in enumerate(remaining):
            ...
            normalized = [
                {
                    ...
                    "size":     f.get("size", ""),
                    "bbox_2d":  f.get("bbox_2d", f.get("bbox", [])),
                }
                for f in parsed
            ]
            ...
            global_idx = all_images_index.get(img_path.name, -1)
            if global_idx >= 0 and (global_idx + 1) % args.batch_size == 0:
    ...
    total_det   = sum(_count(r) for r in all_records)
    errors      = sum(1 for r in all_records if "_error" in r)
    parse_fails = sum(1 for r in all_records if "_raw" in r)
    zero_det    = sum(1 for r in all_records
                      if _count(r) == 0 and "_raw" not in r and "_error" not in r)
