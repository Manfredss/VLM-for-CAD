"""
IMMetric — Enhanced evaluation metric for ms-swift eval_step
Adapted from evaluate_checkpoints_benchmark.py

Metrics reported:
  - detection_f1:        overall F1 (IoU >= threshold + category match)
  - detection_precision:  overall precision
  - detection_recall:     overall recall
  - bbox_iou_mean:        mean IoU of matched predictions
  - size_accuracy:        fraction of matched detections with correct size field
  - per_category F1:      F1 per feature category
  - IMMetric:             legacy combined score (IoU + label match)

Compatible with ms-swift 4.0.0.dev0, Qwen3.5-4B
"""
...
_metric_model_path = os.environ.get('METRIC_MODEL', 'Qwen/Qwen3.5-4B')
_tokenizer = None
...
def normalize_box(box) -> list:
    """Normalize bbox to 0-1000 range, handling various input formats."""
...
def parse_features_from_text(text: str) -> list:
    """Parse feature list from model output text."""
...
def extract_items(features: list) -> list:
    """Extract normalized (category, bbox, size) items from feature list."""
    items = []
    for f in features:
...
def compute_detection_metrics(pred_text: str, label_text: str, iou_threshold: float = IOU_THRESHOLD) -> dict:
    """
    Compute detection-level metrics for a single sample.

    Returns per-sample stats: tp, fp, fn, iou_sum, size_correct, size_total,
    and per-category tp/fp/fn.
    """
    ...
    for pred in pred_items:
        ...
        if best_iou >= iou_threshold and best_idx >= 0:
            ...
            # Quality score (same as benchmark)
            ...
            if cat_match:
                ...
                # Check size accuracy for matched + category-correct detections
                stats["size_total"] += 1
    ...
    # Unmatched GT items are false negatives
    for i, gt in enumerate(gt_items):
...
def compute_task_acc(preds, labels, *, acc_strategy='token',
    ...
    """
    Main metric computation: decode predictions/labels, compute detection metrics.
    Called by ms-swift during evaluation.
    """
    ...
    # Split into individual samples by json blocks
    json_pattern = re.compile(r'```json\s*([\s\S]*?)```', re.MULTILINE)
    pred_blocks = [m.group(1).strip() for m in json_pattern.finditer(pred_text)]
    label_blocks = [m.group(1).strip() for m in json_pattern.finditer(label_text)]

    # Aggregate stats across all samples in the batch
    total = {
        "tp": 0, "fp": 0, "fn": 0,
        "iou_sum": 0.0, "match_count": 0,
        "size_correct": 0, "size_total": 0,
    ...
    if len(label_blocks) == len(pred_blocks) and label_blocks:
        for pred_block, label_block in zip(pred_blocks, label_blocks):
            sample_stats = compute_detection_metrics(
                f"```json\n{pred_block}\n```",
                f"```json\n{label_block}\n```",
            )
            for key in ["tp", "fp", "fn", "iou_sum", "match_count",
                         "size_correct", "size_total", "quality_sum"]:
                total[key] += sample_stats[key]
            for cat, vals in sample_stats["per_cat"].items():
                for k in ["tp", "fp", "fn"]:
                    total["per_cat"][cat][k] += vals[k]
    else:
        # Fallback: treat entire text as single sample
        sample_stats = compute_detection_metrics(pred_text, label_text)
        total = sample_stats
    ...
    metrics = {
        ...
        "IMMetric": avg_quality,  # backward compat
    }
    ...
    for cat in ALL_CATEGORIES:
        ...
        metrics[f"f1_{safe_name}"] = cat_f1
...
# Register as ms-swift 4.x EvalMetrics class (used by --eval_metric IMMetric)
...
# Legacy registration for older ms-swift versions
