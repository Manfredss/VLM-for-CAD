# Industrial Drawing Feature Recognition - Training Template

基于 **Qwen3-VL-3B** 的工业图纸工件特征识别 fine-tune 训练模版。采用 LoRA 微调方案，支持在 RTX 5070 Ti (16GB) 上训练，也支持部署到 Nautilus (Kubernetes) 集群。
...
retrain model/
├── configs/
│   └── train_config.yaml       # 训练配置文件（超参、模型、数据路径等）
├── src/
│   ├── dataset.py              # 数据集加载与预处理（支持 Label Studio 格式）
│   ├── prepare_data.py         # 原始 JSON 转 train/val 划分
│   ├── train.py                # 训练主脚本
│   ├── inference.py            # 推理脚本（单张/批量）
│   ├── merge_lora.py           # LoRA 权重合并工具（部署用）
│   └── evaluate.py             # 评估脚本：训练曲线摘要 + 检测指标对比
├── scripts/
│   └── run_train.sh            # 训练启动脚本（支持多卡）
├── docker/
│   └── Dockerfile              # Docker 镜像构建
├── k8s/
│   └── training_job.yaml       # Nautilus K8s Job + PVC 配置
├── data/
│   ├── example_train.json            # 示例：通用格式
│   └── example_label_studio_export.json  # 示例：Label Studio 导出格式
├── requirements.txt
└── README.md
```
